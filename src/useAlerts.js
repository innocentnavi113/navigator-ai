...
export function useAlerts() {
    ...
    async function load() {
      try {
        ...
        if (saved?.value) {
          ...
          setNewsAlerts(saved.value.newsAlerts !== false)
        }
  ...
  const scanNews = useCallback(async () => {
    ...
    try {
      ...
      setLatestNews(data.all?.slice(0, 15)        || [])
      setForexNews(data.forexNews?.slice(0, 10)   || [])
      setBtcNews(data.btcNews?.slice(0, 10)       || [])
      setTweets(data.tweets?.slice(0, 10)         || [])
      setTrumpAlerts(data.trumpAlerts?.slice(0, 5) || [])
      ...
      for (const article of (data.trumpAlerts || [])) {
        const id = btoa(encodeURIComponent(article.title || '')).slice(0, 40)
        if (seenIds.has(id) || article.ageHours > 6) continue
        await dbPut('seenNews', { id, seenAt: Date.now() })
        sendNotification('🚨 TRUMP ALERT — Market Impact', article.title, { tag: `trump-${id}`, url: article.link })
        await new Promise(r => setTimeout(r, 500))
      }

      // Forex + BTC high impact
      const important = [...(data.forexNews || []), ...(data.btcNews || [])]
        .filter(a => a.score >= 15 && a.ageHours <= 3).slice(0, 3)
      for (const article of important) {
        const id = btoa(encodeURIComponent(article.title || '')).slice(0, 40)
        if (seenIds.has(id)) continue
        await dbPut('seenNews', { id, seenAt: Date.now() })
        const emoji = article.isBTC ? '₿' : '💱'
        sendNotification(
          `${emoji} ${article.isForex ? 'Forex' : 'BTC'} Alert — ${article.source}`,
          article.title,
          { tag: `news-${id}`, url: article.link }
        )
        await new Promise(r => setTimeout(r, 500))
      }

      // Key trader tweets
      for (const tweet of (data.tweets || []).filter(t => t.ageHours <= 2).slice(0, 2)) {
        const id = btoa(encodeURIComponent(tweet.title || '')).slice(0, 40)
        if (seenIds.has(id)) continue
        await dbPut('seenNews', { id, seenAt: Date.now() })
        sendNotification(`🐦 ${tweet.source} tweeted`, tweet.title, { tag: `tweet-${id}`, url: tweet.link })
        await new Promise(r => setTimeout(r, 500))
      }
    ...
    } catch (e) { console.log('News scan error:', e) }
  }, [sendNotification])
  ...
  const toggleNewsAlerts = useCallback(() => setNewsAlerts(prev => !prev), [])
